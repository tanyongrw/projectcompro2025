...
namespace Player1 {
	...
	void Cornering()
	{
		gState.player1->pos += dir_addition[gState.player1->correction] * player_speed;
		bool done = false;
	...
	void PlayerMovement()
	{
		...
		if (try_dir != NONE &&
			!PlayerTileCollision(try_dir, gState.player1->pos) &&
			!InTunnel(gState.player1->pos)) {
			gState.player1->cur_dir = try_dir;
			if (gState.player1->cur_dir != opposite_dir[gState.player1->cur_dir]) {
		...
		if (!gState.player1->stopped) {
			gState.player1->pos += dir_addition[gState.player1->cur_dir] * player_speed;
		}
...
namespace Player2 {
	...
	void Corrnering()
	{
		gState.player2->pos += dir_addition[gState.player2->correction] * player_speed;
		bool done = false;
		switch (gState.player2->correction)
		{
		case UP:
