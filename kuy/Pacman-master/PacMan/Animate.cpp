...
sf::IntRect GetGhostFrame(GhostType type, TargetState state, Dir dir)
{
    ...
    if (state != GOHOME && state != ENTERHOME)
        ghost.left += animate1.ghost_frame_2 * 32;
...
void AnimateUpdate(int ms_elapsed)
{
    ...
    if (gState.player1->stopped && !animate1.death_animation) {
        animate1.pacman_frame = 0;
        animate1.assending = true;
    }
    else {
        animate1.pacman_timer += ms_elapsed;
    }
    ...
    if (animate1.death_animation) {
        if (animate1.pacman_timer > 100) {
            animate1.pacman_timer = 0;
            animate1.pacman_frame++;
        }
        if (animate1.pacman_frame > 10)
            gState.player1->enable_draw = false;
    }
    else if (animate1.pacman_timer > 25 && !gState.player1->stopped) {
        animate1.pacman_frame += (animate1.assending) ? 1 : -1;
        animate1.pacman_timer = 0;
    }
...
sf::IntRect GetPacManFrame(Dir dir, int player)
{
    ...
    Animation* anim = (player == 1) ? &animate1 : &animate2;
...
{
    animate1.pacman_frame = 0;
    animate1.death_animation = false;
    animate2.pacman_frame = 0;
...
{
    animate1.pacman_frame = 1;
    animate1.death_animation = false;
    animate2.pacman_frame = 1;
    animate2.death_animation = false;
    animate1.ghost_frame_2 = false;
    animate2.ghost_frame_2 = false;
}

void PulseUpdate(int ms_elapsed)
{
    animate1.pulse_timer += ms_elapsed;
    if (animate1.pulse_timer > animate1.pulse_limit) {
        animate1.pulse = !animate1.pulse;
        animate1.pulse_timer = 0;
    }
...
{
    animate1.pulse_limit = ms;
    animate2.pulse_limit = ms;
}

bool IsPulse()
{
    return animate1.pulse;
}