...
void Init() //*
{
	...
	pl1->stopped = true;
	gState.player1.push_back(pl1); // เพิ่มผู้เล่นลงใน vector
...
{
	gState.pause_time -= ms_elapsed;
	if (gState.player1_lives < 0 && gState.player2_lives < 0) {
		if (gState.players_lives < 0) {
