...
sf::Vector2f AvaUpdate(Ghost& ghost)
{
	return gState.player->pos;
}
sf::Vector2f MoodengUpdate(Ghost& ghost)
{
	return gState.player->pos + dir_addition[gState.player->cur_dir] * 4;
}
sf::Vector2f NoppoUpdate(Ghost& ghost)
{
...
sf::Vector2f MheenoeyUpdate(Ghost& ghost)
{
	sf::Vector2f target = gState.player->pos;

	// if Mheenoey is 8 tiles away, target player, else target corner
...
void UpdateDirection(std::vector<Dir> squares, Ghost& ghost)
{
	...
	case CHASE:
		...
		{
		case RED:
			target = AvaUpdate(ghost);
			break;
		case PINK:
			target = MoodengUpdate(ghost);
			break;
		case BLUE:
			target = NoppoUpdate(ghost);
			break;
		case ORANGE:
			target = MheenoeyUpdate(ghost);
			break;
		}
		update_dir = true;
		break;
...
void UpdateGhosts()
{
	for (int i = 0; i < 4; i++) {
		...
		if (ghost->target_state == GOHOME &&
			...
			ghost->pos.x = 14;
			gState.energizer_time = fright_time * 1000000000000000000;
...
//000
void AdjustGhostSpeed()
{
	// ชนะด่านแรกให้เพิ่มความเร็วเป็น 0.07
	if (ghost_speed < 0.07f) {
		ghost_speed = 0.07f;
	}
	else {
		//เพิ่มความเร็ว 30%ต่อด่าน
		ghost_speed *= 1.30;
	}

	// จำกัดความเร็วสูงสุด =0.4
	if (ghost_speed > 0.4f) {
		ghost_speed = 0.4f;
	}
}
