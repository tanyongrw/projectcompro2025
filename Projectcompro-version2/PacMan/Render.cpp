...
void DrawGameUI()
{
	...
	if (gState.game_state == MENU || gState.game_state == GAMEOVER) {
		MakeText("Player: " + gState.playerName, 5, 28, { 255, 255, 255 });

		std::string highScore = std::to_string(getHighScore(gState.playerName));
		MakeText("High Score: " + highScore, 5, 32, { 255, 255, 0 });
...

{
	...
	MakeText("Player: " + gState.playerName, 5, 28, { 255, 255, 255 });
	MakeText("Ava", 9, 8, { 255,00,0 });
	MakeText("Moo deng", 9, 11, { 252,181,255 });
	MakeText("Noppo", 9, 14, { 0,255,255 });
	MakeText("Mhee noey", 9, 17, { 248,187,85 });
